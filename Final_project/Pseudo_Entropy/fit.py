...
df9 = pd.read_excel("Q0_N20_m1_1e-5_m2_1e-5.xlsx")
...
def plot(N, n_ini, n_fin, C1, C2, m1, m2):
    ...
    for j in range(0,16):
        ps1_array = np.append(ps1_array, S9_total_list[j])
    ...
    popt1, pcov1 = curve_fit(fit_func, test_t, ps1_array)
    ...
    plt.scatter(n_array, ps9_array, label='data')
    ...
    plt.plot(test_t, fit_func(test_t, *popt1), label='fit: a=%6.4f, b=%6.4f' % tuple(popt1), color='r')

    plt.title('2D_N=' + str(N) + '_m1=' + str(m1) + '_m2=' + str(m2)) 
...
plot(20,0,40,1,100,1e-05,1e-05)
