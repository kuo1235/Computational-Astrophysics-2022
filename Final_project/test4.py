...
def coupling_matrix(L, row_num):
    ...
    for i in range(1, row_num):
        couplingK[i-1][i-1] = (((i - 0.5) ** 2) / ((i + 0.5) ** 2) + (L * (L + 1)) / (i + 0.5)** 2 + 1)
        couplingK[i-1][i] = -((i - 0.5) / (i + 0.5))
        couplingK[i][i-1] = couplingK[i-1][i]

    couplingK[row_num-1][row_num-1] =  (((row_num - 0.5) ** 2) / ((row_num + 0.5) ** 2) + (L * (L + 1)) / (row_num + 0.5)** 2 + 1)
...
def total_entropy(l_max, N, n_ini, n_fin):
    ...
    arealaw.to_excel('Massless.xlsx', sheet_name='sheet1', index=False)
...
areaLaw = total_entropy(10000, 100, 1, 50)
